...
if [ "x${OPT_REBUILD}" != "x1" ]; then
  ...
  docker run -d -t --add-host host.docker.internal:host-gateway --name "${BUILDER_CONTAINER_ID}" "${MOUNT_OPTS[@]}" "${BUILDER_IMAGE_ID_INC}"
else
  #docker run -d -t --name "${BUILDER_CONTAINER_ID}" "${MOUNT_OPTS[@]}" "${BUILDER_IMAGE_ID_BUILDENV}"
  docker run -d -t --add-host host.docker.internal:host-gateway --name "${BUILDER_CONTAINER_ID}" "${MOUNT_OPTS[@]}" "${BUILDER_IMAGE_ID_BUILDENV}"
fi
