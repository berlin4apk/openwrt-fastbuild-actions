...
echo "azure-cli zulu* hhvm llvm* firefox microsoft-edge* google-cloud-sdk google* dotnet* powershell openjdk* temurin-*-jdk mysql*" | xargs --max-args=1 --no-run-if-empty $SudoE $Eatmydata apt-get purge --no-upgrade --no-install-recommends --no-install-suggests --ignore-hold --no-upgrade  ;
_got_more_space
